---
title: "modelling - 2 (no maize flour)"
author: "R Goto"
date: "2023-12-02"
...
## 8.3. Create FCT for status quo and full fortification without maize flour (cooking oils and wheat flour only)
```{r eval=FALSE}
names(TZFCT3)
# excluded ~105 mainze flour fortification (TZFCT3F2)
TZFCT3F2 <- TZFCT3 %>% 
...
# excluded femzfpf, femzfff, znmzfpf, znmzfff, folmzfpf, folmzfff, vb12mzfpf, vb12mzfff
TZFCT3F2 <- TZFCT3F2 %>% 
  mutate(VApf=VAmcg+vaoilpf+vabunpf+vabrepf+vaswepf,
         VAff=VAmcg+vaoilff+vabunff+vabreff+vasweff,
         FEpf=FEmg+fewhfpf+febunpf+febrepf+feswepf,femacpf,
         FEff=FEmg+fewhfff+febunff+febreff+fesweff,femacff,
         ZNpf=ZNmg+znwhfpf+znbunpf+znbrepf+znswepf,znmacpf,
         ZNff=ZNmg+znwhfff+znbunff+znbreff+znsweff,znmacff,
         FOLpf=FOLmcg+folwhfpf+folbunpf+folbrepf+folswepf,folmacpf,
         FOLff=FOLmcg+folwhfff+folbunff+folbreff+folsweff,folmacff,
         VB12pf=VB12mcg+vb12whfpf+vb12bunpf+vb12brepf+vb12swepf,vb12macpf,
         VB12ff=VB12mcg+vb12whfff+vb12bunff+vb12breff+vb12sweff,vb12macff)

TZFCT3F2c <- TZFCT3F2 %>% 
  select(itemcode, FEmg, ZNmg, VAmcg, FOLmcg, VB12mcg, FEpf, FEff, ZNpf, ZNff, VApf, VAff, FOLpf, FOLff, VB12pf, VB12ff)
```

## 8.4. Calculate estimate micronutrient intake in status quo and full fortification (without maize flour fortification)
```{r eval=FALSE}
# merge with TZFCT3F2c with estimated food consumption in g
f60afecF <- left_join(f60afec, TZFCT3F2c, by = "itemcode")
...
write_csv(nafetF2, "nafetF2.csv") # nafetF2 (no maize)
...
# weighting
names(dataAw)
nmAF <- left_join(nafetF2, dataAw, by = 'y4_hhid')   # merge with weighting factor
...
## 8.5. prevalence of inadequacy of micronutrient intakes
```{r eval=FALSE}
nafetF2ad <- nafetF2 %>% 
...
# weighting 
nafetFad_w <- left_join(nafetF2ad, dataAw, by = 'y4_hhid')   # merge with weighting factor
